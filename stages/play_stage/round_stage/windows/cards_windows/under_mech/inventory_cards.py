...
# TODO
class InventoryCards(Rectangle):
    def __init__(self,
        ...
        super(InventoryCards, self).__init__(x=x, y=y, size_x=size_x, size_y=size_y)
    ...
    def get_cards_by_mouse_pos(self, pos) -> SkillCard:
        x, y = pos
        x -= self.x0
        y -= self.y0
    ...
    def draw(self):
        ...
        for card in sorted(self.cards, key=lambda c: c.chosen):
            card.draw()
